...
# --- DEFINISI MASALAH ---
# Representasi nama mahasiswa berdasarkan indeks
# 0: Ani
# 1: Budi
# 2: Citra
# 3: Dedi
...
# Representasi Keadaan (Solusi):
# Sebuah tuple 5-elemen, di mana nilai 0 = Kelompok 1, dan nilai 1 = Kelompok 2.

def hitung_biaya(solusi: tuple) -> int:
    """
    Menghitung total biaya (jumlah pelanggaran batasan) dari sebuah solusi.
    Tujuan adalah mencapai biaya = 0.
    """
    biaya = 0
...
def buat_solusi_awal() -> tuple:
    """Menghasilkan satu solusi (keadaan) awal secara acak."""
    return tuple(random.randint(0, 1) for _ in range(len(MAHASISWA)))

def dapetin_semua_tetangga(solusi: tuple) -> list[tuple]:
    """
    Mendapatkan semua keadaan tetangga dari solusi saat ini.
    Tetangga didefinisikan sebagai solusi yang berbeda 1 elemen (1 mahasiswa pindah grup).
    """
    list_tetangga = []
...
def cetak_solusi(solusi: tuple, biaya: int, nama_algoritma: str):
    """Mencetak hasil akhir dari algoritma dengan format yang jelas."""
...
# --- ALGORITMA UTAMA: HILL CLIMBING ---

def hill_climbing(max_percobaan: int = 100):
    """
    Implementasi algoritma Hill Climbing dengan Random Restart.
    """
    solusi_terbaik = None
